...
エラーが出てRが終了してしまう場合は、[インストール](Installation.html)のStep 6で確認したモデルのパスを指定してください。
```{r, include = FALSE}
...
```{r, warning=FALSE, message=FALSE}
pos(sentence,  keep = c("動詞", "形容詞"))
```

`pos()`関数は、`list`を返しますが、`list`の各要素を`data.frame`とすることができます。
```{r, warning=FALSE, message=FALSE}
pos(sentence, keep = c("動詞", "形容詞"), format = "data.frame")
```
...
pos(texts, format = "data.frame",  keep = c("動詞", "形容詞"))
```
...
```{r, warning=FALSE, message=FALSE}
pos_simple(texts, keep = c("動詞", "形容詞"), format = "data.frame")
```
...
```{r, warning=FALSE, message=FALSE}
tokenize(sentence, keep = c("動詞", "形容詞"), concat = FALSE)
lemmatize(sentence,  keep = c("動詞", "形容詞"), concat = FALSE)
```

これらの関数も複数の文章を処理することもできます。
```{r, warning=FALSE, message=FALSE}
tokenize(texts, keep = c("動詞", "形容詞"), concat = TRUE)
lemmatize(texts,  keep = c("動詞", "形容詞"), concat = FALSE)
```
...
t_tbl <- tokenize_tbl(documents, "text")
l_tbl <- lemmatize_tbl(documents, "text", keep = c("動詞", "名詞"))
```
...
pos_list_furrr <- future_map(
  split_index,
  function(index) {
    pos(data[index, ][["text"]], keep = c("動詞", "名詞"))
  },
...
l_tbl_furrr <- future_map_dfr(
  split_index,
  function(index) {
    lemmatize_tbl(data[index, ], "text", keep = c("動詞", "名詞"))
  },
  .options = furrr_options(packages = c("RcppJagger", "tibble"), seed = TRUE)
)
